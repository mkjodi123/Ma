...
int main(int argc, char *argv[]) {
    ...
    if (local->tm_year + 1900 > EXPIRATION_YEAR ||
        ...
        printf("Expired. khatam ho gya h developar se connect kr @rajaraj_04.\n");
        return EXIT_FAILURE;
    }

    if (argc != 6) {
        printf("Usage: %s [IP] [PORT] [DURATION] [PACKET] [THREAD_COUNT]\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    printf("chudai khatam. RAJABHAI: %lu\n", total_packets_sent);
